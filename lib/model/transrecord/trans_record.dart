...
abstract class TransRecordModelDao {
  ...
  @Query('SELECT * FROM ' +
      tableName +
      ' WHERE  chainid = :chainid and transType =:transType ORDER BY date DESC')
  Future<List<TransRecordModel>> queryMinersTrxListWithType(
      int chainid, int transType);

  @Query('SELECT * FROM ' +
      tableName +
      ' WHERE  chainid = :chainid and (transStatus = 2 OR transStatus = 3)  ORDER BY date DESC')
  Future<List<TransRecordModel>> queryMinersPendingTrxList(int chainid);
